...
class SiteUserManager(BaseUserManager):
    ...
    def create_staffuser(
        self, email, first_name, last_name, user_type, password=None, dob=None
    ):
        user = self.create_user(
            email=email,
            first_name=first_name,
...
class SiteUser(AbstractBaseUser):
    ...
    user_type = models.CharField(
        choices=USER_TYPE_CHOICES, max_length=20, default=UserType.EMPLOYEE
    )
...
class AdminUserManager(SiteUserManager):
    def get_queryset(self):
        return (
            super(AdminUserManager, self)
            .get_queryset()
            .filter(user_type=USER_TYPE_CHOICES[2][0])
        )
...
class PartnerUserManager(SiteUserManager):
    def get_queryset(self):
        return (
            super(PartnerUserManager, self)
            .get_queryset()
            .filter(user_type=USER_TYPE_CHOICES[1][0])
        )
...
class EmployeeUserManager(SiteUserManager):
    def get_queryset(self):
        return (
            super(EmployeeUserManager, self)
            .get_queryset()
            .filter(user_type=USER_TYPE_CHOICES[0][0])
        )
