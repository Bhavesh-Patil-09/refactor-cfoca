...
from placc.models import (
    PLACC,
    Mapping_Company_tally_notes,
    Company,
    NOTES,
    REPORTS,
    Mapping_Company_notes_report,
    Tally_mapping,
    AgeingReport,
    Budget,
)
...
class SignupView(APIView):
    def post(self, request):
        ...
        if SiteUser.objects.filter(email=email).exists():
            ...
            user_ser = SiteUserListSerializer(user)
            return Response(
                {"message": "Sign-Up Successful!!", "user": user_ser.data},
...
class PasswordResetView(APIView):
    def get(self, request, param):
        try:
            ...
            return Response(
                {"message": "Password reset link sent to your email {0}".format(email)},
                status=status.HTTP_200_OK,
            )
        except SiteUser.DoesNotExist:
            return Response({"error": "User not found!"}, status=status.HTTP_200_OK)
    ...
    if os.path.exists(file_path):
        with open(file_path, "rb") as fh:
            response = HttpResponse(
                fh.read(), content_type="application/vnd.microsoft.portable-executable"
            )
...
# Dabtors Aging Report For the period
def darftp(request):
    ageing = AgeingReport.objects.all()
    context = {"ageing": ageing}
    return render(request, "tables/DARFTP.html", context)


# Dabtors Aging Report For the period (Customer wise Aggregation)
def darftp_test(request):
    ageing2 = AgeingReport.objects.all()
    ageing = AgeingReport.objects.values("customer_name").annotate(
        data_sum=Sum("amount")
    )
    for item in ageing:
        print(item["customer_name"])
        print(item["data_sum"])
        item["cust_list"] = AgeingReport.objects.filter(
...
def fdfbo(request):
    sales_actual_till_date = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Sales Accounts")
    ).aggregate(Sum("final"))

    direct_exp = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Direct Expenses")
    ).aggregate(Sum("final"))
    indirect_exp = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Indirect Expenses")
    ).aggregate(Sum("final"))
    purchase = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Purchase Accounts")
    ).aggregate(Sum("final"))
    direct_income = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Direct Incomes")
    ).aggregate(Sum("final"))

    indirect_income = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Indirect Incomes")
    ).aggregate(Sum("final"))
    ...
    if direct_income["final__sum"] != None:
        gross_margin = (
            sales_actual_till_date["final__sum"]
            + direct_income["final__sum"]
            - direct_exp["final__sum"]
        )
    else:
        gross_margin = sales_actual_till_date["final__sum"] - direct_exp["final__sum"]

    net_profit = (
        gross_margin - indirect_exp["final__sum"] + indirect_income["final__sum"]
    )

    SundryDebtors = PLACC.objects.filter(
        tally_mapping_primary="Sundry Debtors"
    ).aggregate(Sum("final"))
    sales_no_days = (sales_actual_till_date["final__sum"] / 30) * 365
    no_of_days = round(
        (SundryDebtors["final__sum"] / sales_actual_till_date["final__sum"]) * 365, 2
    )

    SundryCreditors = PLACC.objects.filter(
        tally_mapping_primary="Sundry Creditors"
    ).aggregate(Sum("final"))

    Inventories = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(description="Opening Stock")
    ).aggregate(Sum("final"))
    salary = PLACC.objects.filter(tally_mapping_parent="SALARY").aggregate(Sum("final"))
    INTEREST = PLACC.objects.filter(description="INTEREST ON TDS").aggregate(
        Sum("final")
    )
    major_expenses = (
        PLACC.objects.filter(
            ~Q(final=None),
            ~Q(final=0.0000),
            Q(tally_mapping_primary="Indirect Expenses")
            | Q(tally_mapping_primary="Direct Expenses"),
        )
        .order_by("-final")
        .reverse()[0:4]
    )
...
def ffsftp(request):
    bank_acc = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Bank Accounts")
    ).aggregate(Sum("final"))
    cash_in_hand = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Cash-in-hand")
    ).aggregate(Sum("final"))
    opening_stock = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(description="Opening Stock")
    ).aggregate(Sum("final"))
    receivables = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(description__icontains="Receivable")
    ).aggregate(Sum("final"))
    trade_payables = PLACC.objects.filter(
        ~Q(final=None),
        ~Q(final=0.0),
        Q(description__icontains="payable"),
        ~Q(tally_mapping_primary="Duties & Taxes"),
        ~Q(tally_mapping_primary="Current Liabilities"),
    ).aggregate(Sum("final"))
    other_assets = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary__icontains="Asset")
    ).aggregate(Sum("final"))
    loan_advance = PLACC.objects.filter(
        ~Q(final=None),
        ~Q(final=0.0),
        Q(tally_mapping_primary="Loans & Advances (Asset)"),
    ).aggregate(Sum("final"))
    deposits = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Deposits (Asset)")
    ).aggregate(Sum("final"))
    current_liabilities = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Current Liabilities")
    ).aggregate(Sum("final"))
    duties_taxes = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Duties & Taxes")
    ).aggregate(Sum("final"))
    # header==================================================
    loans_header = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary__icontains="Loans")
    ).aggregate(Sum("final"))
    ...
    capital_acc_header = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Capital Account")
    ).aggregate(Sum("final"))

    fixed_asset_header = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Fixed Assets")
    ).aggregate(Sum("final"))
    total_assets = (
        opening_stock["final__sum"]
        + bank_acc["final__sum"]
        + cash_in_hand["final__sum"]
        + receivables["final__sum"]
        + loan_advance["final__sum"]
        + deposits["final__sum"]
        + other_assets["final__sum"]
    )
    total_liabilities = (
        current_liabilities["final__sum"]
        + trade_payables["final__sum"]
        + duties_taxes["final__sum"]
    )
    context = {
...
def lbob(request):
    ...
    sundry_debtors = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Sundry Debtors")
    ).aggregate(Sum("final"))
    bank_acc = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(tally_mapping_primary="Bank Accounts")
    ).aggregate(Sum("final"))
    ...
    expense_payable = PLACC.objects.filter(
        ~Q(final=None), ~Q(final=0.0), Q(description="Expenses Payable")
    ).aggregate(Sum("final"))
    ...
    taxes = PLACC.objects.filter(
        ~Q(final=None),
        ~Q(final=0.0),
        (
            Q(description="APMC Tax")
            | Q(description="Deferred Tax")
            | Q(description="Deffred Tax Expenses")
            | Q(description="Income Tax Expenses")
            | Q(description="PROPERTY TAXES")
            | Q(description="VAT LIABILITY A/C")
        ),
    ).aggregate(Sum("final"))
    context = {
        "opening_stock": opening_stock["final__sum"],
        ...
        "bank_acc": bank_acc["final__sum"] + cash_in_hand["final__sum"],
        "a_value": opening_stock["final__sum"]
        + sundry_debtors["final__sum"]
        + bank_acc["final__sum"]
        + cash_in_hand["final__sum"],
        ...
        - (SundryCreditors["final__sum"] + taxes["final__sum"]),
        "SundryCreditors": SundryCreditors["final__sum"],
...
def notemapping(request):
    tally_rec_mapping = (
        PLACC.objects.order_by()
        .values_list("tally_mapping_primary", flat=True)
        .distinct()
    )
    ...
    tally_rec_mapping = (
        PLACC.objects.order_by()
        .values_list("tally_mapping_parent", flat=True)
        .distinct()
    )
...
def mappingcomp(request):
    ...
    default_mapping_list = Tally_mapping.objects.all().values_list(
        "name"
    )  # PLACC.objects.all().values_list('tally_mapping')
    default_mapping_final_list = []
    objects = []
    company_name_fk = Company.objects.get(
        Company_Name="NOBLE RESOURCES AND TRADING INDIA PRIVATE LIMITED"
    )
    company_name_pk = Company.objects.get(
        Company_Name="NOBLE RESOURCES AND TRADING INDIA PRIVATE LIMITED"
    ).pk
    ...
    if request.method == "POST" and "tally_mapping" in request.POST:
        ...
        if (
            Mapping_Company_tally_notes.objects.filter(
                Q(Tally_Mapping=Tally_mapping.objects.get(name=tally))
            ).count()
            > 0
        ):
            item = Mapping_Company_tally_notes.objects.get(
                Tally_Mapping=Tally_mapping.objects.get(name=tally)
            )
            ...
            item.Company_Name = Company.objects.get(
                Company_Name="NOBLE RESOURCES AND TRADING INDIA PRIVATE LIMITED"
            )
    ...
    if request.method == "POST" and "report_mapping" in request.POST:
        ...
        note = request.POST.get("note_mapping")
        if (
            Mapping_Company_notes_report.objects.filter(
                Q(Notes_Mapping=NOTES.objects.get(name=note))
            ).count()
            > 0
        ):
            item = Mapping_Company_notes_report.objects.get(
                Notes_Mapping=NOTES.objects.get(name=note)
            )
            # item.Company_Name = Company.objects.get(Company_Name='NOBLE RESOURCES AND TRADING INDIA PRIVATE LIMITED')
...
def plandbl(request):
    ...
    for i in list2:
        ...
        total = PLACC.objects.filter(tally_mapping_primary=i.Tally_Mapping).aggregate(
            Sum("final")
        )["final__sum"]
        item = Mapping_Company_tally_notes.objects.get(
            Tally_Mapping=Tally_mapping.objects.get(name=i.Tally_Mapping)
        )
        item.amt = total
        item.save(
            update_fields=[
        ...
        total = Mapping_Company_tally_notes.objects.filter(
            Notes_Mapping=i.Notes_Mapping
        ).aggregate(Sum("amt"))["amt__sum"]

        item = Mapping_Company_notes_report.objects.get(
            Notes_Mapping=NOTES.objects.get(name=i.Notes_Mapping)
        )
...
def plandbl_test_old(request):
    ...
    for i in list2:
        ...
        total = Mapping_Company_notes_report.objects.filter(
            Notes_Mapping=i.Notes_Mapping
        ).aggregate(Sum("amt"))["amt__sum"]
        item = Mapping_Company_notes_report.objects.get(
            Notes_Mapping=NOTES.objects.get(name=i.Notes_Mapping)
...
def plandbl_test(request):
    ...
    list_notes = PLACC.objects.filter(~Q(final=None), ~Q(final=0.0)).values_list(
    ...
    for item in final_list:
        ...
        sum_list.append(
            PLACC.objects.filter(
                ~Q(final=None), ~Q(final=0.0), tally_mapping_primary=item[0]
            )
        )

    sum_data = (
        PLACC.objects.filter(~Q(final=None), ~Q(final=0.0))
        .values("tally_mapping_primary")
        .annotate(data_sum=Sum("final"))
    )
    this_lis_date = []
    this_lis_sum = []
    for i in sum_data:
...
def notes(request):
    travelling = PLACC.objects.filter(
        tally_mapping_primary="Travelling and coveyance"
    ).aggregate(Sum("final"))["final__sum"]
    rent = PLACC.objects.filter(tally_mapping_primary="Rent").aggregate(Sum("final"))[
        "final__sum"
    ]
    sc = PLACC.objects.filter(tally_mapping_primary="Share Capital").aggregate(
        Sum("final")
    )["final__sum"]
...
def notes_test_old(request):
    list_reports = Mapping_Company_notes_report.objecdts.all().values_list(
        "Report_Mapping"
    )
    final_list = []
...
def notes_test(request):
    ...
    for item in final_list:
        ...
        sum_list.append(
            PLACC.objects.filter(
                ~Q(final=None), ~Q(final=0.0), tally_mapping_parent=item[0]
            )
        )

    sum_data = (
        PLACC.objects.filter(~Q(final=None), ~Q(final=0.0))
        .values("tally_mapping_parent")
        .annotate(data_sum=Sum("final"))
    )
    this_lis_date = []
    this_lis_sum = []
        ...
        "perticular": [
            {
                ...
                "meeting": {"title": "Demo Meeting", "id": 11},
            },
            {
                "id": 5,
