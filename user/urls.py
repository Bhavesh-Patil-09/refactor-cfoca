...
urlpatterns = [
    ...
    path(
        "api/v1/site_user_list/<str:user_id>/", UsersByUser.as_view()
    ),  # All users under <user_id>
    ...
    path("", home, name="home"),  # Only [GET]
    path("login/", get_login, name="login"),  # Incomplete [POST]
    path("register/", register, name="register"),  # Incomplete [POST]
    path("mis/", mis, name="mis"),  # Only [GET]
    path("darftp/", darftp, name="darftp"),
    path("darftp_test/", darftp_test, name="darftp"),
